<template>
  <div class="scroll-view">
    <PermissionRequest v-if="request.request" :url="request.request.senderURL" action="" object="sign a transaction" />
    ...
    <div style="position: relative">
      ...
      <div class="overlay-dialog" :class="{visible: status=='success'}">
        ...
        <h2>The transaction has been signed.</h2>

        <p v-if="signedTx">
          <strong>Signature: {{lastTxSignature}}</strong><br>
        </p>
  ...
  methods: {
    async confirm () {
      ...
      const result = await timedAsync(() =>
          promisifySimple(this.$background.signTransaction)(this.signedTx, this.signedTx.chainId),
